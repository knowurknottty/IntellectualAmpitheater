...
def _valid_posterior(p) -> bool:
    """Fail-closed input validation: a malformed posterior is rejected outright.
    A VALID but extreme posterior is disagreement (dissent), never Byzantine."""
...
class FusionResult:
    ...
    def quarantined(self) -> list[str]:
        """Deprecated wave-02 alias for `rejected` (malformed sources only)."""
        return self.rejected


def _default_prior(hypotheses: list[str], universe: list[str]) -> dict[str, float]:
    # WAVE-03 (W3-C2): the prior is uniform over the ENUMERATED hypotheses only.
    # The open-world residual is applied post-hoc as a calibration floor (see
    # _fuse_with), never as a competitor inside the log-linear sum.
    enumerated = [h for h in universe if h != OPEN]
    share = 1.0 / max(1, len(enumerated))
    return {h: share for h in enumerated}
...
def fuse(
    ...
    *,
    ...
    def _fuse_with(active: list[Source]) -> dict[str, float]:
        ...
        #   h ABSENT from a source's support  -> neutral: no contribution.
        # WAVE-03 (W3-C2): normalize over the ENUMERATED hypotheses ONLY, then
        # apply the open-world residual as a calibration floor. Letting __open__
        # compete inside the sum let it win every contradiction (a hard p=0
        # contributes log(EPS) = -13.8, crushing enumerated mass below the
        # residual) — exposed by F2/F4 once the wave-02 quarantine bug was fixed.
        enumerated = [h for h in universe if h != OPEN]
        logp = {h: math.log(max(prior.get(h, EPS), EPS)) for h in enumerated}
        for s in active:
            w = weights[s.provenance.source_id]
            for h in enumerated:
                if h in s.posterior:
                    logp[h] += w * math.log(max(s.posterior[h], EPS))
        p = _normalize({h: math.exp(v) for h, v in logp.items()})
        out = {h: (1.0 - P_RESIDUAL) * v for h, v in p.items()}
        out[OPEN] = P_RESIDUAL
        return out
    ...
    # --- WAVE-03 REPAIR (F14): Byzantine exclusion is DECOUPLED from legitimate
    # epistemic disagreement. Wave-02 quarantined any source whose support-restricted
    # symmetric KL to the fused posterior exceeded theta=3.0 nats; an honest strong
    # dissenter (0.9/0.1 against 0.1/0.9) sits at ~3.516 nats and was therefore
    # EXCLUDED from the fusion. That erases exactly the minority the firewall exists
    # to protect. Exclusion is now reserved for MALFORMED posteriors only (input
    # validation, fail-closed). Extreme-but-valid disagreement is reported as an
    # `outlier` and remains in the fusion with its weight already capped.
    rejected = [s.provenance.source_id for s in sources if not _valid_posterior(s.posterior)]
    outliers: list[str] = []
    for s in sources:
        if s.provenance.source_id in rejected:
            continue
        if _source_distance(s, fused) > theta:
            outliers.append(s.provenance.source_id)

    active = [s for s in sources if s.provenance.source_id not in rejected]
    if rejected:
            ...
            dissent.append(
                {
                    ...
                    "rejected": any(
                        s.provenance.source_id in rejected for s in group
                    ),
                }
            )
